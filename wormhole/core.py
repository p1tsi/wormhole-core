...
class Core(object):
    ...
    def standard_modules(self) -> List[str]:
        ...
        logger.info(f"Getting standard hooking modules: {standard_modules}")
    ...
    def _on_message(self, message: dict, data: bytes) -> None:
        ...
        """
        if message.get('payload', {}).get('type', '') == "download":
            self._handle_file_download(message, data)
        else:
    ...
    def _handle_file_download(self, msg: dict, data: bytes) -> None:
        """
        Function responsible for receiving and saving the data of downloaded 
        files with fs commands. The file is saved inside the appData directory,
        named as the session value generated by the agent.
        :param msg: the JSON msg coming from the agent signalling 
                    the beginning, the incoming data and the end
                    of data transmission
        :param data: the actual data of the file being downloaded
        """

        if msg.get('payload').get('event') == 'data':
            with open(os.path.join(self._data_dir, msg.get('payload').get('session')), "ab") as outfile:
                outfile.write(data)
            self._script.post({"type": "ack"})